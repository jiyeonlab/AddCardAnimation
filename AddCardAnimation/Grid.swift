...
struct Grid
{
    ...
    // SetCardView에서 호출 시, 어떤 layout으로 할건지와 frame에 무엇이 들어가는지 넣어줌.
    // SetCardView의 bounds를 Grid의 frame으로 넣었음.
    ...
    // 이거때문에 SetCardView에서 layout[index]로 cellFrame에 접근할 수 있음.
    ...
    // layout을 어떤 걸로 하느냐에 따라 rowCount, columnCount, cellSize를 지정해줌. 중요한 부분.
    private mutating func recalculate() {
        ...
        case .dimensions(let rowCount, let columnCount):
            if columnCount > 0 && rowCount > 0 {
                ...
                // view안에 들어갈 column, row를 지정해주면, 그에 따른 카드하나(cell)의 사이즈가 나옴.
                // 여기서쓰인 frame은 SetCardView에 해당함.
    ...
    // recalculate()에서 호출한 곳.
    // cellSize에 따라, boundingSize, Offset, origin 등을 지정해줌.
